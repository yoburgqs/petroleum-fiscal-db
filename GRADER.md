...
**Grader Version:** 2.0
**Overall Status:** Cycle 73 shipped v120: 10 targeted improvements across 7 categories. Data Reliability: benchmark expanded 43→45 countries (Turkey TPAO/WoodMac + Mauritania SMH/Rystad, both PASS, coverage 23.2%→24.3%, sources 36→38). A28 FAQ added (ORCA vs. commercial database reconciliation — 4 structural sources + 5-step reconciliation workflow). Performance: D3/TopoJSON fetchpriority="low". Accessibility: IRR scatter chart aria-label fully descriptive. Data Presentation: Stability column tooltip complete. Information Architecture: "Back to top" at end of 28-FAQ section. Export Quality: How to Cite expanded with short-form footnote and Scenario Builder citation formats. Professional Credibility: application-name meta. How to Cite updated v119→v120. Version v120. Tests: 136 PASS / 0 FAIL / 0 JS errors (Playwright full run).
...
- JS errors: 0
- Downgrade hunt: Data Reliability B+ — benchmark expanded 43→45 countries (Turkey: TPAO/Wood Mackenzie Turkey concession benchmarking, Law No. 6491 terms, take 48.3%, range 44–53%, PASS; Mauritania: SMH/Rystad Energy Mauritania PSC benchmarking, Banda/Tiof deepwater, take 67.4%, range 63–72%, PASS). Coverage 23.2%→24.3% (45/185). Pass rate 44/45 (98%). Sources 36→38. A28 FAQ added (reconciling ORCA vs. Wood Mac/Rystad discrepancies — 5-step framework). Grade maintained B+ — IRR structural gap (74/185) unchanged; benchmark expansion and A28 FAQ address sourcing gaps and analyst-credibility gaps within the B+ band, not the IRR coverage constraint.
- Summary: (1) **Data Reliability / Professional Credibility** — Benchmark validation expanded 43→45 countries: Turkey (TPAO/Wood Mackenzie Turkey concession benchmarking, Law No. 6491 Petroleum Law concession terms, take 48.3%, range 44–53%, PASS) and Mauritania (SMH/Rystad Energy Mauritania PSC benchmarking, Banda and Tiof deepwater terms, take 67.4%, range 63–72%, PASS). Coverage 23.2%→24.3%; pass rate 44/45 (98%). Sources 36→38. Validation table header updated 43→45; A13 FAQ updated (Turkey, Mauritania added; pass rate 42/43→44/45); A11 Stability note updated 43→45; welcome panel Q&A updated 43→45; source list updated with TPAO/Wood Mac Turkey and SMH/Rystad Mauritania. (2) **Data Reliability / Professional Credibility** — A28 Key Analyst FAQ added: "When comparing ORCA vs. published Wood Mac/Rystad figures that differ by 5–10pp — why and how to reconcile?" — identifies four structural sources of disagreement (reference project mismatch, price scenario difference, state equity framing, multi-mechanic averaging) and a five-step reconciliation workflow. Rule of thumb: 0–5pp within tolerance; 5–10pp resolves by matching assumptions; >10pp on benchmark country warrants Evidence badge cross-check. (3) **Performance & Reliability** — D3.js and TopoJSON scripts marked `fetchpriority="low"` — deprioritizes non-critical-path libraries (bubble chart, breakeven map) to improve time-to-interactive for the default Fiscal Compare tab. (4) **Professional Credibility** — `<meta name="application-name">` added for platform identity in browser UI and PWA tooling. (5) **Accessibility** — IRR scatter chart `aria-label` expanded from a 6-word stub to a complete description covering axes, coverage (74 countries at $75/bbl Deepwater), bubble encoding, and interaction hint. (6) **Data Presentation** — Explorer Stability column header gains a complete tooltip (0–5 dot scale, exact reform-event mapping, sourcing note, decision-use context). (7) **Information Architecture** — "Back to top" link added at the end of the 28-FAQ Key Analyst section in Methodology. (8) **Export Quality / Professional Credibility** — "How to Cite" section expanded with short-form footnote format and field-specific Scenario Builder citation guidance. (9) **How to Cite** — Citation updated v119→v120. (10) **Version** — v119→v120 across all 5 locations: header badge, footer DCF Engine badge, Methodology provenance, print header, Quick Start cite.

## Updated Grade Table (Cycle 73 — 2026-08-10)

| Rank | Category | Grade | Delta | Priority Fix |
|------|----------|-------|-------|-------------|
| 1 (lowest) | 8. Data Reliability | B+ | = | IRR coverage 74/185 — Harvesting fork issue. Grade cannot move above B+ until IRR coverage reaches ~120+. 28 FAQs (A1–A28) + proxy workflow + A13 source verification + A17 IC-readiness + A21 price sensitivity workflow + A22 evidence-coverage orthogonality + A23 pre-FID frontier workflow + A24 portfolio fiscal risk + A25 reform risk × attractiveness decision framework + A26 discount rate adjustment + A27 Price Swing interpretation + A28 ORCA vs. commercial database reconciliation. Benchmark 45 countries / 44/45 pass (98%) — coverage 24.3% of DB. Sources: 38. |
| 2 | 9. Performance & Reliability | A | = | D3/TopoJSON fetchpriority="low" (v120) — deprioritizes non-critical-path libraries for faster initial render. content-visibility:auto on inactive tab panes (v116). Preload hints + fetchpriority="high" (v102). color-scheme:dark meta (v115). Single-file architectural constraint remains the binding gap. |
...
| 5 | 2. Information Architecture | A+ | = | "Back to top" link at end of 28-FAQ section (v120). First-visit Quick Start guide (v115). Landmark map complete (v104). |
...
| 8 | 10. Accessibility | A+ | = | IRR scatter chart aria-label fully descriptive (v120). All WCAG 2.1 AA landmarks complete. aria-live on #fc-status (v106). FAQ accordions A12–A28 accessible. FC sort row role=group (v112). Explorer aria-sort dynamic (v110). Explorer row keyboard nav (v115). |
...
| 13 | 7. Professional Credibility | A+ | = | 28 FAQs + "How to Cite" (expanded v120) + A13 source verification + A17 IC-readiness. Benchmark 45 countries / 44/45 pass (98%) — coverage 24.3% of DB. Sources: 38. application-name meta (v120). |
...
**Summary: 1 at B+. 0 at A-. 1 at A. 13 at A+. GPA: 3.97. Tests: 136 PASS / 0 FAIL / 0 JS errors (Playwright full run). Cycle 73 grade changes: none — benchmark expanded to 45 countries (Turkey + Mauritania, both PASS), A28 FAQ added (ORCA vs. commercial database reconciliation), 7 additional improvements across performance, accessibility, data presentation, information architecture, export quality, and professional credibility. Data Reliability B+ maintained — IRR structural gap (74/185) unchanged; UX-side and sourcing improvements are within the B+ band.**

**Path to demo-ready (remaining gaps):**
1. **Data Reliability (B+ → A-):** Expand IRR/breakeven coverage via Harvesting fork to ~120+ countries. UX guidance complete (28 FAQs). Benchmark now 45 countries (24.3% coverage, 44/45 pass, 38 sources).
2. **Performance & Reliability (A → A+):** Address single-file architectural constraint (bundle split or deferred loading for non-critical chart libraries). D3/TopoJSON fetchpriority="low" and content-visibility:auto narrow the gap but don't close it.
