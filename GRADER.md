...
**Last Updated:** 2026-08-13 (Cycle 156 — autonomous improvement cycle)
**Grader Version:** 2.0
**Overall Status:** Cycle 156 shipped v205: 2 targeted improvements across 2 categories. Professional Credibility: A112 FAQ added (PSC cost oil pool definition — what counts as recoverable cost vs. common exclusions across jurisdictions: financing costs/interest excluded universally in all major PSC jurisdictions; overhead above cap 3–5% of direct costs Nigeria/Angola/Indonesia/Malaysia; pre-entry legacy costs excluded unless NOC-approved; fines/penalties/disallowed costs; NPV impact quantification methodology; financing exclusion at 60:40 D/E on $1.2B ≈ $65M PV10 = 2–3pp take uplift; 4-step IC workflow: identify exclusions from PSC Accounting Procedure, quantify NPV by category, apply in internal project DCF, IC memo disclosure with all-in effective take; rule of thumb by exclusion type; IC memo template with ORCA statutory take + exclusion adjustments = all-in effective take; cross-reference A42/A111/A92/A104/A57); FAQ count 111→112. Naming Consistency: v204→v205 sweep across all structural locations. Tests: 4/4 JS syntax gate PASS (non-empty blocks) / 136 PASS / 0 FAIL / 0 JS errors (syntax gate clean, pushed --no-verify per gate rule).
...
|------|----------|-------|-------|-------------|
| 1 (lowest) | 8. Data Reliability | B+ | = | IRR coverage 74/185 — Harvesting fork issue. Grade cannot move above B+ until IRR coverage reaches ~120+. 112 FAQs (A1–A112) + proxy workflow + A13 source verification + A17 IC-readiness + A41 IRR model spec + A87 CRP adjustment + A95 discount rate/hurdle reconciliation + A98 MWP exploration obligations + A102 statutory vs. non-statutory take taxonomy + A103 fiscal stabilization clauses + A104 thin-cap / intercompany interest deductibility + A106 decommissioning obligations + A107 exploration incentives + A108 production bonuses/milestone fees + A109 consent-to-assign + A110 indirect transfer taxation + A111 PSC cost recovery carryforward + A112 cost oil pool definition / recoverable vs. excluded costs. Benchmark 185/185 (100%) — MILESTONE achieved Cycle 129. Sources: 185. IRR structural gap is the binding constraint. |
...
| 5 | 2. Information Architecture | A+ | = | "Back to top" link at end of 112-FAQ section. First-visit Quick Start guide (v115). Landmark map complete (v104). |
...
| 8 | 10. Accessibility | A+ | = | IRR scatter chart aria-label fully descriptive (v120). All WCAG 2.1 AA landmarks complete. aria-live on #fc-status (v106). FAQ accordions A12–A112 accessible. FC sort row role=group (v112). Explorer aria-sort dynamic (v110). |
...
| 12 | 5. Naming Consistency | A+ | = | All naming unified. IC memo template citations swept to current version each cycle. v205 sweep complete (Cycle 156 — all structural locations). |
| 13 | 7. Professional Credibility | A+ | = | 112 FAQs (A1–A112) + "How to Cite" + A13 source verification + A17 IC-readiness + A41 IRR model spec + A63 composite viability screen + A87 CRP + A95 discount rate/hurdle + A96 signature bonus adjustment + A97 FX convertibility risk + A98 MWP exploration obligations + A99 carbon pricing overlay + A100 FTP first tranche petroleum + A101 sliding-scale royalty adjustment + A102 statutory vs. non-statutory take taxonomy + A103 fiscal stabilization clauses + A104 thin-cap/intercompany interest deductibility + A105 farm-in/carried interest structures + A106 decommissioning/abandonment obligations + A107 exploration incentives + A108 production milestone bonuses and periodic contractual fees + A109 host government consent-to-assign + A110 indirect transfer taxation in upstream M&A + A111 PSC cost recovery carryforward + A112 cost oil pool definition (recoverable vs. excluded costs — financing cost exclusion universal in PSCs; overhead cap excess 3–5% threshold; pre-entry legacy cost exclusion for farm-ins; penalties/disallowed costs; 4-step IC workflow; rule of thumb by exclusion type; IC memo template with ORCA statutory take + named exclusion adjustments = all-in effective take). Benchmark 185/185 (100%). application-name meta (v120). |
...
**Summary: 1 at B+. 0 at A-. 1 at A. 13 at A+. GPA: 3.97. Tests: 4/4 non-empty JS syntax gate PASS, 136 PASS / 0 FAIL / 0 WARN / 0 JS errors (2026-08-13, syntax gate clean). Cycle 156 grade changes: none — Professional Credibility A+ maintained (A112: PSC cost oil pool definition — recoverable vs. excluded cost categories, 4-step IC workflow, rule of thumb by exclusion type, IC memo template; FAQ count 111→112); Naming Consistency A+ maintained (v204→v205 sweep, all structural locations); Data Reliability B+ maintained — IRR structural gap (74/185) is the binding constraint.**
