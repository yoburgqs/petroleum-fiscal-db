...
**Last Updated:** 2026-08-14 (Cycle 175 — A129 WHT/repatriation FAQ + IC memo template citations + 8 categories)
**Grader Version:** 2.0
**Overall Status:** Cycle 175 shipped v226: 10 improvements across 8 categories. Professional Credibility: A129 FAQ added — withholding tax and dividend repatriation risk (WHT rates by jurisdiction, FX risk tier classification, 4-step IC workflow, full-stack investor friction taxonomy); FAQ count 128→129. Naming Consistency: IC memo template citations updated from ORCA v219 → v226 in 5 FAQ template-language instances (A115, A116, A120×2, A121, A122). Performance: reform_history.json preload priority high→low (not on critical path). Data Reliability: IRR coverage tooltip expanded with source specificity and Harvesting fork expansion note. Export: How to Cite updated to v226. Naming: v225→v226 sweep across 6 structural locations. IA: Methodology card updated to 129 analyst FAQs. Tests: 4/4 JS syntax gate PASS / 0 JS errors (pre-commit verification).
...
| 8 | 10. Accessibility | A+ | = | IRR scatter chart aria-label fully descriptive (v120). All WCAG 2.1 AA landmarks complete. aria-live on #fc-status (v106). FAQ accordions A12–A129 accessible. FC sort row role=group (v112). Explorer aria-sort dynamic (v110). |
