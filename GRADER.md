...
|------|----------|-------|-------|-------------|
| 1 (lowest) | 8. Data Reliability | B+ | = | IRR coverage 74/185 — Harvesting fork issue. Grade cannot move above B+ until IRR coverage reaches ~120+. 15 FAQs + proxy workflow + verification workflow cover all UX-side gaps. Benchmark 27 countries / 26/27 pass (96%). |
| 2 | 6. Error & Empty States | A | = | All empty states informative. Screener zero-results upgraded in v102. Reform Risk no-reform empty state upgraded in v105. clearSavedScenarios two-step inline confirmation is correct UX. |
| 3 | 9. Performance & Reliability | A | = | Preload hints + fetchpriority="high" (v102). preconnect (v104). will-change:transform (v105). runFiscalCompare null guard (v106). Single-file constraint remains architectural. |
